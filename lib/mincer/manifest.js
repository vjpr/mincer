...
prop(Manifest.prototype, 'paths_cache', [], {writable: true});
 ...
 *      });
 **/
Manifest.prototype.compile = function (files, options, callback) {
  var self     = this,
      paths    = [];
  ...
  options.onlyLogicalPaths = options.onlyLogicalPaths || false;
  ...
  if (!paths.length) {
    ...
    if (!options.onlyLogicalPaths) {
      console.time('eachLogicalPath')
      this.environment.eachLogicalPath(files, function (pathname) {
        paths.push(pathname);
      });
      console.timeEnd('eachLogicalPath')
    } else {
      paths = files;
    }
